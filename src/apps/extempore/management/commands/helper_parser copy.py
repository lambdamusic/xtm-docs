...
def _parse_onefile(f, original_path, IGNORE_CONSTANTS=True, IGNORE_SYMBOLS=True, githubUrl=None):
	"""Extract definitions from a single Extempore file.
	Returns a list of dicts, one per function.
	If the function comments are present right after the function definition, they are picked up automatically.

	We assume that valid functions definitions are separated by a blank line (at least). 
	The function keywords accepted are defined at the top of this file.

	We iterate through a list of lines, and for each line we check if it starts with one of the keywords.
	If it does, we extract the function name and type and keep it in a buffer. The line is added to another
	buffer and we keep adding lines till the next blank line or function start keyword.
	...
	----------
	f : string
		Filepath to the file to be parsed.
	TODO
